jQuery( function ( $ ) {
	...
	$( '.js-ocdi-import-data' ).on( 'click', function () {
		...
		if ( $('#ocdi__options-file-upload').length ) {
			data.append( 'options_file', $('#ocdi__options-file-upload')[0].files[0] );
			data.append( 'options_option_name', $('#ocdi__options-option-name').val() );
		}
	...
	$( '.select-import-type' ).on( 'change', function(){
		var selected_val = $(this).val();
		$(this).parent().find( '.js-ocdi-gl-import-data' ).attr( 'data-layout-demo', selected_val );
	});
	...
	$( '.js-ocdi-gl-import-data' ).on( 'click', function () {
		...
		var selectedImportType = $( this ).data( 'layout-demo' );
		...
		if ( ocdi.import_popup ) {
			displayConfirmationPopup( selectedImportID, selectedImportType, $itemContainer );
		}
		else {
			gridLayoutImport( selectedImportID, selectedImportType, $itemContainer );
		}
	});
	 ...
	 */
	function gridLayoutImport( selectedImportID, selectedImportType, $itemContainer ) {
		// Reset response div content.
		$( '.js-ocdi-ajax-response' ).empty();
		...
		data.append( 'importtype', selectedImportType );
	 ...
	 * @param obj $itemContainer The jQuery selected item container object.
	 */
	function displayConfirmationPopup( selectedImportID, selectedImportType, $itemContainer ) {
