			...
			<div id="<?php echo esc_attr( $id_widget ); ?>" class="search input-group" 
				...
				<div class="cat-wrapper">
					<label class="label-search">
						<span><?php echo esc_html__( 'Select Category','sw_ajax_woocommerce_search' ) ; ?></span>
						<select name="category" class="s1_option category-selection">
							<option value=""><?php echo esc_html__( 'All Categories','sw_ajax_woocommerce_search' ) ; ?></option>
				...
				<div class="content-search">
					<span class="text-search"><?php  echo esc_html__( 'Search for anything','sw_ajax_woocommerce_search' ) ; ?></span>
					<input class="autosearch-input" type="text" value="<?php echo esc_attr( ( isset( $_GET['s'] ) && $_GET['s'] ) ? ( $_GET['s'] ) : "" ); ?>" size="50" autocomplete="off" placeholder="<?php echo esc_attr__( 'Bike, Accessories, Brands, Events, Holidays etc.', 'sw_ajax_woocommerce_search' ); ?>" name="s">	
					<div class="search-append"></div>
				</div>
					<span class="input-group-btn">
					<button type="submit" class="button-search-pro form-button"><?php  echo esc_html__( 'Search','sw_ajax_woocommerce_search' ) ; ?></button>
				</span>
