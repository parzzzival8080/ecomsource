...
if ( ! class_exists( 'WC_Currency_Converter' ) ) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			$this->settings = array(
				array(
					'name' => esc_html__( 'Open Exchange Rate API', 'sw_woocommerce' ),
					'type' => 'title',
					'desc' => '',
					'id'   => 'woocommerce-currency-converter-widget'
				),
				array(
					'name' => esc_html__( 'App Key', 'sw_woocommerce' ),
					'desc' => sprintf( __( '(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'sw_woocommerce' ), 'https://openexchangerates.org/signup' ),
		...
		public function admin_notice_wrong_key() {
			?>
			<div class="error">
				<p><?php esc_html_e( 'WooCommerce Currency Converter: Incorrect key entered!', 'sw_woocommerce' ); ?></p>
			</div>
		<?php
		}
			...
			if ( $currencies ) {
				...
				if ( ! empty( $instance['currency_display'] ) && 'select' === $instance['currency_display'] ) {
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= ' <a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'sw_woocommerce' ) . '</a>';
					}
					...
					if ( ! empty( $instance['show_reset'] ) ) {
						$html .= '<li><a href="#" class="wc-currency-converter-reset reset">' . esc_html__( 'Reset', 'sw_woocommerce' ) . '</a></li>';
					}
		...
		public function enqueue_assets() {
			...
			// Scripts
			wp_register_script( 'moneyjs', WCURL . '/includes/currency-converter/assets/js/money.min.js', array( 'jquery' ), null, true );
			wp_register_script( 'accountingjs', WCURL . '/includes/currency-converter/assets/js/accounting.min.js', array( 'jquery' ), null, true );
			wp_register_script( 'wc_currency_converter', WCURL . '/includes/currency-converter/assets/js/conversion.js', array(
				'jquery',
				'moneyjs',
				'accountingjs',
			), null, true );
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'            => esc_html__( 'Original price:', 'sw_woocommerce' ),
