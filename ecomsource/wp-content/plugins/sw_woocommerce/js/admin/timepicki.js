...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			if(settings.reset){
				$( "<a class='reset_time'>x</a>").insertAfter( new_ele );
			}
	...
	var sale_price_date_from = $( '#woocommerce-product-data' ).find( '#_sale_price_dates_from' ).val();
	var sale_price_date_to	 = $( '#woocommerce-product-data' ).find( '._sale_price_dates_to' ).val();
	alert( sale_price_date_from );
	
	$( '#woocommerce-product-data' ).on( 'click', '.sale_schedule', function() {
		var $wrap = $( this ).closest( 'div, table' );

		$( this ).hide();
		if( sale_price_date_from != '' || sale_price_date_to != '' ){
