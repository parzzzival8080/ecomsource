 ...
 * Register Widgets
**/
function sw_wooswatches_register_widgets() {
	unregister_widget( 'WC_Widget_Layered_Nav' );
	register_widget( 'Custom_WC_Widget_Layered_Nav' );
}
add_action( 'widgets_init', 'sw_wooswatches_register_widgets' );
...
class Custom_WC_Widget_Layered_Nav extends WC_Widget {
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		// List display.
		echo '<ul class="woocommerce-widget-layered-nav-list-'.$taxonomy.' custom-filter-layered">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$term_html = '<a rel="nofollow" href="' . esc_url( $link ) . '" title="'. esc_attr( $term->name ) .'"><span class="style-color"><span '.$attr2 .'></span></span><span class="name-color">' . esc_attr( $term->name ) . '<span></a>';
			} else {
				$link      = false;
				$term_html = '<span '.$attr2 .'>' . esc_attr( $term->name ) . '<span>';
			}
