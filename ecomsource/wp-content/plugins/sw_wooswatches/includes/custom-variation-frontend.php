...
?>
	...
	<?php foreach( $attributes as $key => $attribute ){	?>
		...
		<?php if( taxonomy_exists( $key ) ){ ?>				
			...
			?>
					<label class="radio-label sw-radio-variation sw-radio-variation-<?php echo esc_attr( $i )  ?>" title="<?php echo esc_attr( $term->name ) ?>">
						<span <?php echo $attr2; ?>><?php echo $term->name; ?></span>
					</label>
			<?php
					}
				}
				...
				foreach( $attribute as $j => $option ){
					...
					$variation_color2 = isset( $meta_variation[$key]['color2'][$j] ) ? $meta_variation[$key]['color2'][$j] : '';
