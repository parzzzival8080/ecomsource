			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $key => $term ) {
					...
					if ( in_array( $term->slug, $options ) ) {
						$html .= '<label class="radio-label sw-radio-variation sw-radio-variation-color sw-radio-variation-'. esc_attr( $key .' '. $active ) .'" title="'. esc_attr( $term->slug )  .'" for="'. esc_attr( $term->slug . '_' . $key ) . '" '. $parent_attr .'>';
						...
						$html .= '<span '. $attr2 .'>'. $term->name .'</span>';
						$html .= '</label>';
					}
				}			
			}else {
				...
				foreach ( $options as $key => $option ) {					
					...
					$html .= '<label class="radio-label sw-radio-variation sw-radio-variation-image sw-radio-variation-'. esc_attr( $key .' '. $active ) .'"  title="'. esc_attr( $option )  .'"  for="'. esc_attr( $id ) . '" '. $parent_attr .'>';
					$html .= '<input type="radio" id="'.  esc_attr( $id ) .'" name="' . esc_attr( $name ) . '" data-attribute_name="attribute_' . esc_attr( sanitize_title( $attribute ) ) . '" value="' . esc_attr( $option ) . '" '. $checked .'/>';
