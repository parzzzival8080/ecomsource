...
// Don't print empty markup if there's nowhere to navigate.
		...
		if ( !empty( $pre_post ) ):
			...
			<a href="<?php echo get_the_permalink( $pre_post->ID ); ?>">
				...
				<span><i class="xs xs-long-arrow-alt-left-solid"></i><?php esc_html_e( 'Previous Post', 'marketo' ) ?></span>
			</a>
		...
		if ( !empty( $next_post ) ):
			...
			<a href="<?php echo get_the_permalink( $next_post->ID ); ?>">
				...
				<span><?php esc_html_e( 'Next Post', 'marketo' ) ?> <i class="xs xs-long-arrow-alt-right-solid"></i></span>
			</a>
...
if ( !function_exists( 'marketo_post_meta' ) ) {
	...
	function marketo_post_meta() {
		...
		if ( get_post_type() === 'post' ) {
			...
			if ( is_sticky() ) {
				echo '<span class="meta-featured-post sticky"> <i class="xs xs-thumbtack-solid"></i> ' . esc_html__( 'Sticky', 'marketo' ) . ' </span>';
			}
			if ( is_single() ) {
				// Get the post author.
			...
			if ( $category_list ) {
				echo '<span class="meta-categories post-cat"> <i class="xs xs-folder-solid"></i>   ' . $category_list . ' </span>';
			}
			...
			if ( is_single() ) {
				// Comments link.
				if ( comments_open() ) :
					echo '<span class="post-comment"><i class="xs xs-comment-alt-regular"></i> ';
...
if ( !function_exists( 'marketo_paging_nav' ) ) {
	...
	function marketo_paging_nav() {
		...
		if ( get_previous_posts_link() )
			printf( '<li>%s</li>' . "\n", get_previous_posts_link( '<span class="xs xs-angle-left-solid"></span>' ) );
		...
		if ( get_next_posts_link() )
			printf( '<li>%s</li>' . "\n", get_next_posts_link( '<i class="xs xs-angle-right-solid"></i>' ) );
...
function marketo_link_pages() {
	$args = array(
		...
		'separator'			 => '  ',
		'nextpagelink'		 => esc_html__( 'Next ', 'marketo' ) . '<I class="xs xs-angle-right-solid"></i>',
		'previouspagelink'	 => '<I class="xs xs-angle-left-solid"></i>' . esc_html__( ' Previous', 'marketo' ),
	);
	wp_link_pages( $args );
}
