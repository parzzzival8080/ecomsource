...
class Xs_Loging_Logout_Widget extends Widget_Base {

    public function get_name() {
        return 'xs-login-logout';
    }

    public function get_title() {
        return esc_html__( 'Marketo Login Logout', 'marketo' );
    }
    ...
    protected function _register_controls() {
        $this->start_controls_section(
            'section_tab',
            [
                'label' =>esc_html__('Marketo Login Logout', 'marketo'),
            ]
        );
    ...
    protected function render( ) {
        ...
        <?php endif; ?>
        <?php if(is_user_logged_in() && !empty($first_name)): ?>
            <div class="user-wrapper">
                <h4 class="user-name"><?php echo esc_html($first_name);?><span><i class="xs xs-angle-down-solid"></i></span></h4>
                <a class="user-logout" href="<?php echo wp_logout_url( home_url() ); ?>">Logout</a>
            </div>
        <?php endif; ?>
        <?php
    }
