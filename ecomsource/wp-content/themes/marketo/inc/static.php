...
if ( !is_admin() ) {
    ...
    //custom icons
    wp_enqueue_style( 'elementor-icons-icon-marketo', MARKETO_CSS . '/xs-icon-font.css', null, MARKETO_VERSION );
    //Swiper CSS
    wp_enqueue_style( 'swiper', MARKETO_CSS . '/swiper.min.css', null, MARKETO_VERSION );
...
if ( !is_admin() ) {
    ...
    //Swiper JS
    wp_enqueue_script( 'swiper', MARKETO_SCRIPTS . '/swiper.min.js', array('jquery'), MARKETO_VERSION, TRUE );
    
    // Font face observer
    wp_enqueue_script( 'fontfaceobserver', MARKETO_SCRIPTS . '/fontfaceobserver.js', array(), true, true );

	wp_enqueue_script( 'marketo-main', MARKETO_SCRIPTS . '/main.js', array( 'jquery' ), MARKETO_VERSION, true );
    wp_enqueue_script( 'marketo-ajax-setting', MARKETO_SCRIPTS . '/ajax-script.js', array('jquery'), MARKETO_VERSION, TRUE );
...
// add_filter( 'style_loader_tag',  'marketo_preload_filter', 10, 2 );
// function marketo_preload_filter( $html, $handle ){
//     if (strcmp($handle, 'marketo-plugins') == 0) {
